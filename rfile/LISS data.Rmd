...
With the code below we read panel data year by year, starting with 2023 and keep only the variables of interest.
...
Notice that some observations encoded with -9 are impossible and for the propensity to vote there are 525 missing values. Similarly for political orientation there are 421 missing values. We have to choice but to drop these from our sample.
...
Below we recode the political orientation data such that LEFT=0 for values from 0 to 5 (including), and RIGHT=1 for values from 6 to 10. 
...
And we are left with 1633 observations.
...
In our panel 2019-2023, 841 respondents were left-leaning and 792 were right leaning.
